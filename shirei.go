...
// AnyHovered reports whether the pointer is over any hoverable container at
// all. A host that composites Shirei over content of its own (a game drawing
// its UI above the scene) asks this to decide whether a click belongs to the
// UI or to what is behind it. Like IsHovered, it reads the hit chain built at
// frame start, so it answers for the frame that is on screen.
//
// The root container is hoverable and covers the window, so it always ends the
// hit chain; only a hit on something inside it counts as the UI.
