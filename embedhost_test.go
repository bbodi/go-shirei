...
// Embedding a UI over a host's own content needs two things core did not have:
// a transparent clear, so the host sees through what the frame did not paint,
// and a way to tell a click on the UI from a click on the scene behind it.
...
func embedRender(t *testing.T, transparent bool) *Framebuffer {
	...
	return rend.Render(out.Surfaces, 400, 300, 1)
}
