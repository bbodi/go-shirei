...
// clearTransparent zeroes the buffer, so every pixel the frame does not paint
// carries alpha 0. See SoftRenderer.Transparent.
...
type SoftRenderer struct {
	...
	// Transparent clears the un-painted pixels to alpha 0 instead of opaque
	// white, for a host that composites the frame over content of its own — a
	// game drawing its UI over the scene, say. The output stays premultiplied,
	// so the host blends it as dst = src.rgb + dst*(1-src.a). Off by default:
	// a window that presents the buffer directly wants the opaque canvas.
	Transparent bool
...
// SurfacePaints reports whether a surface puts anything on the screen: a
// glyph, an image, a border, a gradient, or a fill that is not fully
// transparent. Structure — a clip push, a transparency group — paints nothing
// by itself.
//
// A host that composites a frame over its own picture scans the frame's
// surfaces with this to tell "the UI drew something" from "the UI built only
// structure", which it cannot read off the surface count: the root container
// always emits one.
